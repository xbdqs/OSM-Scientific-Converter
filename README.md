...
The five manuscript figures, source data, and reproducible scripts are stored in `paper/figures/`. Figure 2 is assembled only from screenshots captured from the packaged English v0.4.1 Windows application during the Berlin power workflow. The repository does not use translated, redrawn, or simulated GUI panels. Public validation summaries are stored in `validation/public/`; regional PBF inputs and private project databases are intentionally excluded.
