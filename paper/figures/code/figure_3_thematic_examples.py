...
def draw(data_file: Path, output_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig(output_dir/'Figure_3_thematic_examples.png',dpi=600,bbox_inches='tight',facecolor='white')
    fig.savefig(output_dir/'Figure_3_thematic_examples.pdf',bbox_inches='tight',facecolor='white')
    plt.close(fig)

if __name__=='__main__':
    root=Path(__file__).resolve().parents[1]
    p=argparse.ArgumentParser(); p.add_argument('--data',type=Path,default=root/'data/Figure_3_thematic_category_data.csv'); p.add_argument('--output-dir',type=Path,default=root/'output')
